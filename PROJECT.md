...
- [x] now.html: the 4 aircraft stat cards (altitude/speed/heading/distance) could end up visibly different sizes — `fitText()` was shrinking each independently, and "Heading" (e.g. "WSW ↖" — letters plus a rotated arrow glyph) is simply wider than a plain number like "693", so it got shrunk more than its neighbours even though all four are meant to read as one uniform row. First tried a group-fit (`fitTextGroup()`, sizing all four together at whatever fits the widest) — wrong call: it dragged the three fine cards down to heading's size instead, making everything small when there was plenty of room. Reverted to independent `fitText()` per card and instead cut heading's actual overhead (dropped the literal space before the arrow glyph, shrank the arrow from 0.7em to 0.55em, tightened the flex gap from 0.2em to 0.08em) so it no longer needs to shrink at all in the common case — all four now render at full, equal size
